defmodule DurableServer.StorageBackend.Migrating do
  @moduledoc """
  Dual-backend adapter for live storage migration.
           ...
           |-- hit
                 ...
                 `-- promote_on_fallback=true
                     ...
                     put into read_preference
                        ...
                        `-- transient failure          -> return fallback object
  ```
  ...
  During migration, truth can intentionally shift phase by phase.
  ...
  Phase 2: Read Cutover
    read_preference=:secondary, write_target=:primary (or :secondary)
    keep mirror/fallback/promotion on until confidence is high

  Phase 3: Write Cutover
    read_preference=:secondary, write_target=:secondary
    keep mirror_writes=true briefly for rollback safety, then disable
  ...
  def normalize_opts(opts) when is_list(opts) do
    ...
    unless read_preference in @preference do
      raise ArgumentError, "migrating backend :read_preference must be :primary or :secondary"
    end

    unless write_target in @preference do
      raise ArgumentError, "migrating backend :write_target must be :primary or :secondary"
    end

    unless mirror_mode in @mirror_modes do
      raise ArgumentError,
            "migrating backend :mirror_mode must be :best_effort or :required"
    end
        ...
        case StorageBackend.get_object(fallback_backend, key, opts) do
          {:ok, %{body: body} = obj} ->
            if state.promote_on_fallback do
              promote_fallback_object(read_backend, key, body, opts, obj)
            else
              {:ok, obj}
            end
  ...
  defp promote_fallback_object(read_backend, key, body, read_opts, fallback_obj) do
    ...
    case StorageBackend.put_object(read_backend, key, body, max_retries: 3) do
      ...
      {:error, _reason} ->
        # If promotion fails transiently, still return data to preserve read availability.
        {:ok, fallback_obj}
    end
  end
