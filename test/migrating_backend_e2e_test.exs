defmodule DurableServer.MigratingBackendE2ETest do
  use ExUnit.Case, async: false

  import DurableServer.TestHelper

  alias DurableServer.Backends.{EKVStore, MigrationStore, ObjectStore}
  ...
  setup do
    ...
    prefix = "migrating_e2e/#{unique_id}/"
    ekv_name = :"durable_migrating_e2e_#{unique_id}"
    data_dir = Path.join(System.tmp_dir!(), "durable_server_migrating_e2e_#{unique_id}")
  ...
  test "strict phase 1 write fails when mirrored EKV write fails", context do
    unique_id = System.unique_integer([:positive, :monotonic])
    ekv_name = :"durable_migrating_write_fail_#{unique_id}"
    supervisor_name = unique_supervisor_name("shadow_write_fail")
    prefix = "migrating_e2e/write_fail/#{unique_id}/"
    key = "shadow-write-fail"
    data_dir = Path.join(System.tmp_dir!(), "durable_server_migrating_write_fail_#{unique_id}")
  ...
  defp unique_supervisor_name(label) do
    :"durable_migrating_#{label}_#{System.unique_integer([:positive, :monotonic])}"
  end
