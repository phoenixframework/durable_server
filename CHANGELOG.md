## 0.1.5 (unreleased)
- Re-evaluate live owner transitions in `ensure_started_child/3` until the caller's overall `:timeout` instead of giving up after a separate hard-coded five-second Group registration wait. The retry loop re-reads storage with bounded backoff, returns replacement Group owners, and safely retries CAS-backed startup after an old lock is released.
